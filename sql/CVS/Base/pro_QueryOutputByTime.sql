/******************************************************
 *	得到符合条件送货单列表(包括所有信息) 包含销售出货与退货出货
 *	Author: shinbo.chen
 *  date: 2009/12/6
 *******************************************************/ 
CREATE PROCEDURE pro_QueryOutputByTime
		  					@start			varchar(20),	
							@end				varchar(20)			
 AS
...
select  
	...
	tc_shortname,
	troh1_remark
from table_return_output_head where  troh1_time >= @start and troh1_time <= @end
...
while( exists(select * from #table_tmp) )
	begin
		...
		select  	
						...
						tp_no,
						trol1_amount
		from table_return_output_list where troh1_no=@tsoh1_no
		...
		if( exists(select * from #table_finalList where tsoh1_no = @tsoh1_no ) )
			begin
					update	#table_finalList set
							tsoh1_detail = tsoh1_detail+@detail
					where	tsoh1_no = @tsoh1_no			
			end
		else
			begin
